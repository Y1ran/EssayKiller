...
def article_iterator(tokenizer):
    ...
    for (dirpath, dirnames, filenames) in os.walk(args.input_fn):
        for filename in filenames:
            print("dirpath, " + os.path.join(dirpath, filename))
...
def buffered_and_sliding_window_article_iterator(tokenizer, final_desired_size=513):
    ...
    for article in article_iterator(tokenizer):
        ...
        print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 0", len(article['input_ids']))
        print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 1", final_desired_size)
        while len(article['input_ids']) < final_desired_size:
            article['input_ids'].append(0)
        print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 2", len(article['input_ids']))
        yield article
